...
const thinkingOptions: Record<AgentThinkingLevel, Option> = {
  ...
  medium: { value: 'medium', label: 'Balanced', icon: 'token' },
  high: { value: 'high', label: 'Deep', icon: 'magicpen' },
};
...
const ChatComposer = ({
  ...
  const models = useMemo<Option[]>(() => {
    ...
    return [
      defaultModels[0],
      ...withoutAuto.map((model) => ({ value: model.value, label: model.label, icon: 'bot' })),
    ];
  }, [controls?.models]);
