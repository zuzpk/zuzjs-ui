...
const SegmentItem = ({ onSelect, meta, selected }) => {
    ...
    return _jsxs(Button, { onClick: () => onSelect(index, pos.width, pos.x, meta, false), ref: ref, 
        // data-x={pos.x}
        suppressHydrationWarning: true, className: `--segment-item flex aic rel ${selected ? `--segement-active` : ``}`.trim(), children: [icon && _jsx(Box, { className: `--segment-icon ${icon instanceof String ? `icon-${icon}` : `flex aic jcc`}`, children: typeof icon !== `string` && icon }), label && label.trim() != `` && _jsx(Box, { className: `--segment-label`, children: label || `Item ${index}` })] });
};
